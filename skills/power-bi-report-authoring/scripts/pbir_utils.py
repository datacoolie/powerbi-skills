...
def get_alt_text(data: dict) -> str | None:
    ...
    not a per-visual-type "objects" property. Confirmed via
    `powerbi-report-author formatting list-vcos` (general VCO has an altText
    property) vs. `formatting search <type> altText` returning no per-visual-type
    hits for objects.general.
    """
    try:
        v = data.get("visual") or data.get("visualContainer", {}).get("visual", {})
        alt = v.get("visualContainerObjects", {}).get("general", [{}])[0].get("properties", {}).get("altText")
...
def set_alt_text(data: dict, text: str) -> bool:
    ...
    Writes to visual.visualContainerObjects.general[0].properties.altText — the
    shared VCO location, valid for every visual type (unlike objects.general,
    which does not exist for most visual types and rejects altText even where
    a "general" object object does exist, e.g. cardVisual).
    """
    v = data.get("visual") or data.get("visualContainer", {}).get("visual", {})
    if not isinstance(v, dict):
        return False
    vcos = v.setdefault("visualContainerObjects", {})
    general = vcos.setdefault("general", [{}])
    if not general:
        general.append({})
