...
| `slicer` / `filterSlicer` | `header.fontColor/background`, `items.fontColor/background` | [slicers.md](slicers.md) |
| `listSlicer` / `advancedSlicerVisual` | `background`, `label.fontColor`, `value.fontColor`, `data` (button/list family — no `header`/`items`) | [slicers.md](slicers.md) |
...
Modern slicers do **NOT** inherit from the legacy `"slicer"` key, and they split
into two formatting families — add entries for **all four** types:

- `slicer` / `filterSlicer` — set `header.background`, `header.fontColor`,
  `items.background`, `items.fontColor`.
- `listSlicer` / `advancedSlicerVisual` — set `background`, `label.fontColor`,
  `value.fontColor` (these have **no** `header`/`items` objects).

Verify property names per type with
